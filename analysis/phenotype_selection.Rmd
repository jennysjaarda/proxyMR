...
traits_corr2.DT <- traits_corr2 %>% as_tibble() %>% mutate(r = sqrt(r2)) %>% dplyr::select(Neale_pheno_ID, description, variable_type, category, N_pairs, r2, r) %>% unique()
...
Lastly, left-side body traits (highly correlated with right-side) were removed and all 'qualifications' data (corresponding to field [6138](https://biobank.ctsu.ox.ac.uk/showcase/field.cgi?id=6138)) because data is roughly captured with continuous variables (such as years of education).
