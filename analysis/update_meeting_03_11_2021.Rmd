...
household_MVMR_BF.DT <- household_MVMR_summary.DT %>% filter(xi_meta_pval < 0.05/num_tests*2) %>% dplyr::select(exposure_ID, outcome_ID, exposure_description, outcome_description, corr_traits, yi_meta_beta, yi_meta_pval, xi_meta_beta, xi_meta_pval, xp_meta_beta, xp_meta_pval) %>% unique()
...
The plot below displays the direct effect (from MVMR) vs indirect effects (from univariate MR) of $X_i$ on $Y_p$ for all combinations of traits with `abs(correlation) < 0.8` (i.e. the data above plus non-significant results). Total number of tests is `r dim(household_MVMR_summary.DT)[1]/2`. The second plot shows only BF-significant results according to the MVMR. 

```{r direct_vs_indirect_effects_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE}

  plot_data <-  inner_join(household_MR_summary_sub, household_MVMR_summary.DT)  %>% dplyr::select(exposure_ID, outcome_ID, exposure_description, outcome_description, corr_traits, IVW_meta_beta, IVW_meta_se, yi_meta_beta, yi_meta_se, xi_meta_beta, xi_meta_se, xi_meta_pval, xp_meta_beta, xp_meta_se) %>% unique()

sig_plot_data <- plot_data %>% filter(xi_meta_pval < 0.05/num_tests*2)
...
plot_sig <-
  ggplot2::ggplot(data = sig_plot_data, ggplot2::aes(x = IVW_meta_beta,
                                                 y = xi_meta_beta)) +
  geom_point(alpha = 3 / 4) +
  geom_smooth(
    mapping = aes(x = IVW_meta_beta, y = xi_meta_beta),
