...
Decided best to also include all $X$ IVs in the model as well. MVMR model is: $Y_p \sim Y_i + X_i$. 
...
- `PC` is the genetic PC. 
- `trait_couple_corr` is the correlation of `outcome_description` in couples. 
- `PC_couple_corr` is the correlation of `PC` in couples. 
- `trait_PC_corr` is the correlation of `outcome_description` with `PC`.
-  `corr_due_to_confounding` corresponds to the formula: `cor(X,PC)^2*cor(PC_i,PC_p)`.
...
- `outcome_description` corresponds to the trait. 
- `coordinate` is the coordiante (North: 129 or East: 130). 
- `trait_couple_corr` is the correlation of `outcome_description` in couples. 
- `coordinate_couple_corr` is the correlation of `coordinate` in couples. 
- `trait_coordinate_corr` is the correlation of `outcome_description` with `coordinate`.
-  `corr_due_to_confounding` corresponds to the formula: `cor(X,coord)^2*cor(coord_i,coord_p)`.
