...
```{r XiYi_IV_overlap, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE)}
...
yiyp_adj.DT <- fig_dat %>% dplyr::select(exposure_description, outcome_description, exposure_sex, YiXi_IV_sig_overlap, yiyp_IVW_beta, yiyp_IVW_se, yiyp_IVW_pval, yiyp_IVW_beta_adj, yiyp_IVW_se_adj, yiyp_IVW_pval_adj)
...
```{r YiYp_adj_fig_comparison,  echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE}

create_fig_meta <- function(fig_dat, custom_col){
  ...
  plot <- ggplot2::ggplot(data = plot_data, ggplot2::aes(x = yiyp_IVW_meta_beta,
                                                         y = yiyp_IVW_meta_beta_adj, color = factor(overlap_above_50))) +
    geom_point(alpha = 3/4) +

    geom_smooth(mapping = aes(x = yiyp_IVW_meta_beta, y = yiyp_IVW_meta_beta_adj), method = "lm", se=FALSE, formula = y~x+0, fullrange=TRUE, color = custom_col[4]) +
    geom_abline(slope=1, intercept=0, color = "black") +
    scale_color_manual(values = custom_col[c(1,2)], labels=c("overlap =< 50%", "overlap > 50%")) +
    theme_minimal() +
    ggplot2::labs(colour = legend_title, x = paste0("MR estimate\n(Y-index to Y-partner)"),
                  y = paste("AM MR estimate adjusted for\nIV effect on relevant X")) + ggtitle("Comparison of adjusted and unadjusted AM estimated\n(meta-analyzed across sexes)") +
...
create_fig <- function(fig_dat, custom_col){
  ...
  legend_title <- "IV overlap"
  plot <- ggplot2::ggplot(data = plot_data, ggplot2::aes(x = yiyp_IVW_beta,
                                                         y = yiyp_IVW_beta_adj, color = factor(overlap_above_50))) +
    geom_point(alpha = 3/4) +

    geom_smooth(mapping = aes(x = yiyp_IVW_beta, y = yiyp_IVW_beta_adj), method = "lm", se=FALSE, formula = y~x+0, fullrange=TRUE, color = custom_col[4]) +
    geom_abline(slope=1, intercept=0, color = "black") +
    scale_color_manual(values = custom_col[c(1,2)], labels=c("overlap =< 50%", "overlap > 50%")) +
    theme_minimal() +
    ggplot2::labs(colour = legend_title, x = paste0("MR estimate\n(Y-index to Y-partner)"),
                  y = paste("AM MR estimate adjusted for\nIV effect on relevant X"), title = "Comparison of adjusted and unadjusted AM estimated \n (sex-specific results)") + 
    theme(plot.title = element_text(hjust = 0.5))
...
plot_meta <- create_fig_meta(fig_dat, custom_col)
plot <- create_fig(fig_dat, custom_col)
...
## Updated two trait results with adjusted $Y_i$.
...
yi_adj_increase <- which(abs(proxyMR_MR_paths_summary_yiyp_adj$yiyp_IVW_meta_beta) < abs(proxyMR_MR_paths_summary_yiyp_adj$yiyp_IVW_meta_beta_adj))

proxyMR_inv <- proxyMR_comparison_summary_yiyp_adj[yi_adj_increase, ]

t1 <- as.data.frame(table(proxyMR_comparison_summary_yiyp_adj$omega_vs_gam_BF_sig_meta)/2)
t1.2 <- as.data.frame(table(proxyMR_inv$omega_vs_gam_BF_sig_meta)/2)

colnames(t1) <- c("omega_vs_gam_BF_sig_meta", "Frq")

kbl(t1) %>% kable_styling(bootstrap_options = "striped", full_width = F, position = "left")

t2 <- as.data.frame(table(proxyMR_comparison_summary_yiyp_adj$omega_vs_rho_BF_sig_meta)/2)
t2.2 <- as.data.frame(table(proxyMR_inv$omega_vs_rho_BF_sig_meta)/2)
colnames(t2) <- c("omega_vs_rho_BF_sig_meta", "Frq")

kbl(t2) %>% kable_styling(bootstrap_options = "striped", full_width = F, position = "left")

t3 <- as.data.frame(table(proxyMR_comparison_summary_yiyp_adj$omega_vs_gam_rho_BF_sig_meta)/2)
t3.2 <- as.data.frame(table(proxyMR_inv$omega_vs_gam_rho_BF_sig_meta)/2)
