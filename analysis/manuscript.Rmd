...
number_of_potential_confounders <- dim(corr_potential_trait_confounders_pos)[1]/dim(corr_vs_MR_effects_sig_dat)[1]
...
sig_confounders <- corr_potential_trait_confounders_pos %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
...
sig_confounders_top5 <- corr_potential_trait_confounders_pos %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  arrange(-corr_due_to_confounding_ratio) %>%
  mutate(total_potential_confounders = n()) %>%
  slice_head(n = 5) %>%
...
max_C_data <- corr_potential_trait_confounders_pos %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  arrange(-corr_due_to_confounding_ratio) %>%
  mutate(total_potential_confounders = n()) %>%
  slice_head(n = 1) %>%
...
pheno_corr_pearson_sig <- binned_pheno_corr_summary %>% filter(bin_slope_pval_pearson < 0.05/num_tests_by_PCs) %>% arrange(group)
...
```{r, pheno_binned_figs, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), echo = FALSE, out.width = "200%", dpi=600}
...
corr_impact_by_traits_causal_summary <- corr_impact_by_traits_causal %>% group_by(exposure_ID) %>%
   dplyr::summarise(mean_ratio = mean(corr_due_to_confounding_ratio), median_ratio = median(corr_due_to_confounding_ratio), sd_ratio = sd(corr_due_to_confounding_ratio), number_of_traits = n()) 
...
corr_impact_by_traits_causal_summary %>%
  kbl() %>%
  kable_styling()
...
We tested the impact of `trait_interest` on couple correlation amongst all traits in our couple MR framework (number of traits = `r dim(household_MR_summary_AM)[1]`) with the exception of birth place coordinates (2 each [N and E]), we also removed the trait itself from the confounder analysis where applicable (in the case of traits `20016_irnt`, `738` and `845`). [Trait `189_irnt` was not included in our final couple MR analysis but could still be evaluated here because no MR-estimates were used.]
...
```{r PC_confounder_plot, echo = FALSE, eval = FALSE, message=FALSE}
...
```{r coord_confounder_plot, echo = FALSE, eval = FALSE, message=FALSE, out.width = "200%", dpi=600}
...
for(i in 1:length(confounder_traits_sub)){
  ...
  plot <-
    ...
    ggplot2::geom_errorbar(
      ggplot2::aes(xmin = outcome_ID_couple_r - outcome_ID_couple_r_se, xmax = outcome_ID_couple_r + outcome_ID_couple_r_se),
      colour = "grey",
      width = 0
    ) +
...
figures_grid <- plot_grid(plotlist = list(proxyMR_prod_comparison_fig_yiyp_adj, boxplot), nrow=2, ncol = 1, byrow = T)
figures_grid
...
write.xlsx_wrapper(pheno_corr_pearson_sig.DT, file=file_name, sheetName="pheno_binned_corrs", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(corr_impact_by_traits_causal_summary, file=file_name, sheetName="corr_impact_ratios_summary", append=TRUE, row.names=FALSE)
