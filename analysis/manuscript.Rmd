...
pearson_vs_spearman_plot
ggsave("docs/figure/manuscript.Rmd/pearson_vs_spearman_plot.pdf", pearson_vs_spearman_plot, device = cairo_pdf)
...
ggsave("docs/figure/manuscript.Rmd/pheno_corr_plot.pdf", pheno_corr_plot, device = cairo_pdf)
...
corr_vs_MR_effects_plot <- ggplot2::ggplot(data = corr_vs_MR_effects_dat, ggplot2::aes(x = Correlation,
  ...
  #geom_text(aes(x=1,y=2,label=""),show.legend=F) +
  ggplot2::labs(x = paste0("Partner correlation"),
                y = paste("MR-estimate")) +
...
ggsave("docs/figure/manuscript.Rmd/corr_vs_MR_effects_plot.pdf", corr_vs_MR_effects_plot, device = cairo_pdf)
...
sig_confounders <- corr_potential_trait_confounders_pos %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  arrange(-corr_due_to_confounding) %>%
  mutate(total_potential_confounders = n()) %>%
  dplyr::summarize(description = outcome_description[1],
                   
                   total_potential_confounders = total_potential_confounders[1],
                   potential_confounders = paste0(exposure_description, collapse = "; "), 
                   summ_C = sum(corr_due_to_confounding)) %>%
  dplyr::rename(trait_ID = outcome_ID) %>%
  dplyr::rename(trait_description = description) %>% 
  dplyr::select(trait_description, trait_ID, everything())
...
```{r summ_C_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE, fig.height=10, dpi=600}

summ_C_data <- corr_potential_trait_confounders_pos %>% filter(sig_confounder) %>% 
...
sig_confounder_plus_SE <- left_join(sig_confounders, summ_C_data %>% dplyr::select(outcome_ID, corr_due_to_confounding_summ_SE), by = c("trait_ID"="outcome_ID")) %>%
dplyr::rename(summ_C_se = summ_C) 

summ_C_plot <- ggplot2::ggplot(data = summ_C_data, 
  ...
  geom_text_repel(data = subset(summ_C_data, label_phenos==TRUE),
                  nudge_y = -0.25 - subset(summ_C_data, label_phenos==TRUE)$same_trait_corr_minus_MR_est, size = 3.25,
                  direction = "x", show.legend = FALSE, box.padding = 0.5)


summ_C_plot

ggsave("docs/figure/manuscript.Rmd/summ_C_plot.pdf", summ_C_plot, device = cairo_pdf)

## combine `summ_C_plot` with `corr_vs_MR_effects_plot`

corr_vs_MR_comb <- plot_grid(plotlist = list(corr_vs_MR_effects_plot, summ_C_plot), nrow=2, ncol = 1, byrow = T, labels="AUTO")

corr_vs_MR_comb

ggsave("docs/figure/manuscript.Rmd/corr_vs_MR_comb.pdf", corr_vs_MR_comb, device = cairo_pdf)
...
ggsave("docs/figure/manuscript.Rmd/pheno_binned_figures.pdf", pheno_binned_figures, device = cairo_pdf)
...
ggsave("docs/figure/manuscript.Rmd/causal_confounder_plot.pdf", figures_grid, device = cairo_pdf)
...
stat.test <- data_boxplot %>% 
  t_test(beta ~ data, comparisons = cmpr, paired = T) %>%
  mutate(myformatted.p = paste0(pretty_round(p))) %>% 
  mutate(y.position = c(0.6, 0.4, 0.5))
...
ggsave("docs/figure/manuscript.Rmd/boxplot_combined.pdf", figures_grid, device = cairo_pdf)
...
final_table <- df_list %>% reduce(full_join, by='exposure_ID') %>% 
  dplyr::rename(trait_ID = exposure_ID) %>%
  dplyr::rename(trait_description = description) %>%
  dplyr::select(trait_description, trait_ID, variable_type, category, n_pairs, 
                pheno_couple_r2, pheno_couple_r, pheno_couple_r_se, everything()) 
...
#write.xlsx_wrapper(nominally_sig_sex_differences, file=supplementary_tables_file, sheetName="Supp Table 3 nominally_sig_sex_diff", append=TRUE, row.names=FALSE)
...
write.xlsx_wrapper(sig_confounder_plus_SE, file=supplementary_tables_file, sheetName="Supp Table 2 sig_confounders", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(paths_summary, file=supplementary_tables_file, sheetName="Supp Table 3 paths_summary", append=TRUE, row.names=FALSE)
