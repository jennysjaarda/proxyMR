...
pearson_vs_spearman_dat <- inner_join(traits_corr, unique(Neale_SGG_dir_filt_unique[,c("sgg_phesant_name", "variable_type")]), by = c("ID"="sgg_phesant_name")) %>% dplyr::rename(r2_pearson = r2) %>%
...
pearson_vs_spearman_plot <- ggplot2::ggplot(data = pearson_vs_spearman_dat, 
                                            ggplot2::aes(x = r_pearson,
                                                         y = r_spearman, 
  ...
  scale_color_manual(name = "Variable type", values = custom_col[c(1,2,3)], labels=c("Binary","Continuous (IRNT)", "Ordinal"))
...
Sought to compare the raw correlation amongst couples vs. the standardized MR effects. There are many traits where the `correlation > MR effects` including some traits of interest such as standing height, place of birth, among many others. 
...
corr_vs_MR_effects_plot <- ggplot2::ggplot(data = corr_vs_MR_effects_dat, ggplot2::aes(x = Correlation,
  ...
  geom_text_repel(data = subset(corr_vs_MR_effects_dat, label_phenos==TRUE),
                  nudge_y = -0.25 - subset(corr_vs_MR_effects_dat, label_phenos==TRUE)$MR_est, size = 3.25,
...
5. Remove any `potential_counders` with correlation with `trait_ID` > `r corr_trait_threshold`.
6. For each `trait_ID` prune remaining `potential_counders` so that all have correlations <  `r corr_trait_threshold`, prioritized by their `corr_due_to_confounding_ratio`. 

A summary of the top 5 confounders for each `trait_ID` can be seen below: 
...
number_of_potential_confounders <- dim(corr_potential_trait_confounders)[1]/dim(corr_vs_MR_effects_sig_dat)[1]
...
sig_confounders <- corr_potential_trait_confounders %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  ...
  dplyr::summarize(description = outcome_description[1],
                   ...
                   max_C = max(corr_due_to_confounding), 
                   min_C = min(corr_due_to_confounding)) %>%
  dplyr::rename(trait_ID = outcome_ID) %>%
  dplyr::rename(trait_description = description) %>% 
  dplyr::select(trait_description, trait_ID, everything())


sig_confounders_top5 <- corr_potential_trait_confounders %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  arrange(-corr_due_to_confounding_ratio) %>%
  mutate(total_potential_confounders = n()) %>%
  slice_head(n = 5) %>%
...
```{r max_C_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE} 


max_C_data <- corr_potential_trait_confounders %>% filter(sig_confounder) %>% group_by(outcome_ID) %>%
  arrange(-corr_due_to_confounding_ratio) %>%
  mutate(total_potential_confounders = n()) %>%
  slice_head(n = 1) %>%
  ...
  mutate(same_trait_corr_minus_MR_est_se = variance_of_sum(outcome_ID_IVW_se, outcome_ID_couple_r_se))



max_C_plot <- ggplot2::ggplot(data = max_C_data, ggplot2::aes(x = corr_due_to_confounding,
  ...
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = same_trait_corr_minus_MR_est - same_trait_corr_minus_MR_est_se, ymax = same_trait_corr_minus_MR_est + same_trait_corr_minus_MR_est_se),
    colour = "grey",
    width = 0
  ) +
  ggplot2::geom_errorbar(
    ggplot2::aes(xmin = corr_due_to_confounding - Z_prod_se, xmax = corr_due_to_confounding + Z_prod_se),
    colour = "grey",
    width = 0
  ) +

  theme_minimal() +
  ggplot2::labs(x = paste0("max C"),
...
Finally, we computed a MVMR to sum up the effect of all potential confounders as shown in the DAG below and compared these C-statistics to the difference between phenotypic correlation and MR-estimates. 
...
```{r MVMR_C_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE}

discrepentant_traits <- unique(corr_potential_trait_confounders$outcome_ID)
...
for(i in 1:length(potential_trait_confounders_MVMR)){
  ...
  corr_potential_trait_confounders_i <- corr_potential_trait_confounders %>% filter(outcome_ID == !!outcome_ID)
  ...
  MVMR_result_org <- MVMR_result_org %>% mutate(beta_2_var = 4*beta^2*se^2+2*se^4) %>%
    ...
    left_join(corr_potential_trait_confounders_i %>% dplyr::select(exposure_ID, exposure_ID_AM_IVW_beta, exposure_ID_AM_IVW_se), by = c("exposure" = "exposure_ID")) %>%
...
- The table below shows the *nominally significant* results at `p < 0.05` (of which there are `r dim(AM_result_sig_nom.DT)[1]/2`).
...
# write.table(nominally_sig_sex_differences, "docs/manuscript_tables/nominally_sig_sex_differences.txt", row.names = F, sep=",", quote = F)
```
...
We also tested the phenotypic correlation among the different bins. Below are the traits that show a significant trend either with Pearson's or Spearman's correlation (p < `r 0.05/num_tests_by_PCs`). 
...
binned_pheno_corr_sig <- binned_pheno_corr_summary %>% filter(bin_slope_pval_spearman < 0.05/num_tests_by_PCs | bin_slope_pval_pearson < 0.05/num_tests_by_PCs)

binned_pheno_corr_sig.DT <- replace_Neale_ID(binned_pheno_corr_sig, traits_corr2_filled, "trait", "trait_description") %>%
  dplyr::rename(trait_ID = trait) %>%
  dplyr::select(trait_description, trait_ID, everything())

  
format_round_cols <- colnames(dplyr::select_if(binned_pheno_corr_sig.DT, is.numeric))

DT::datatable(binned_pheno_corr_sig.DT, width = "100%") %>% formatRound(columns=format_round_cols, digits = 3)
  
# write.table(binned_pheno_corr_sig.DT, "docs/manuscript_tables/binned_phenotypic_correlations.txt", row.names = F, sep=",", quote = F)
...
corr_impact_by_traits_ratio <- replace_Neale_ID(corr_impact_by_traits_ratio, traits_corr2_filled, "confounder_trait", "confounder_trait_description") %>% ungroup() %>%
...
# write.table(corr_impact_ratios_summary, "docs/manuscript_tables/corr_impact_ratios_summary.txt", row.names = F, sep=",", quote = F)
```
...
confounder_traits_sub <- confounder_traits[c(2,3,4,5)]
...
```{r PC_confounder_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE}
...
plot_PCs <-
  ...
  ggplot2::geom_errorbar(
    ggplot2::aes(ymin = corr_due_to_confounding_all - corr_due_to_confounding_all_se, ymax = corr_due_to_confounding_all + corr_due_to_confounding_all_se),
    colour = "grey",
    width = 0
  ) +
  ggplot2::geom_errorbar(
    ggplot2::aes(xmin = trait_couple_corr - trait_couple_corr_se, xmax = trait_couple_corr + trait_couple_corr_se),
    colour = "grey",
    width = 0
  ) +
...
```{r coord_confounder_plot, echo = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE), message=FALSE, out.width = "200%", dpi=600}

plot_data <- corr_impact_by_coords %>% dplyr::select(Neale_pheno_ID, trait_couple_corr, corr_due_to_confounding_all, corr_due_to_confounding_all_se, trait_couple_corr_se, trait_couple_corr_se) %>% unique()
...
figures_grid <- plot_grid(plotlist = plots, nrow=3, ncol = 2, byrow = T, labels="AUTO", label_size = 12)
figures_grid
...
omega_regression_estimates <- result %>% dplyr::select(outcome, everything())
...
figures_grid <- plot_grid(plotlist = list(proxyMR_prod_comparison_fig_yiyp_adj, boxplot), nrow=1, ncol = 2, byrow = F)
figures_grid
...
file_name <- "docs/manuscript_tables/manuscript_tables.xlsx"
...
write.xlsx_wrapper(corr_vs_MR_effects_sig_dat, file=file_name, sheetName="sig_corr_vs_MR", row.names=FALSE)
write.xlsx_wrapper(sig_confounders, file=file_name, sheetName="sig_confounders", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(AM_result_sig.DT, file=file_name, sheetName="BF_sig_same_trait_MR", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(nominally_sig_sex_differences, file=file_name, sheetName="nominally_sig_sex_diff", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(binned_pheno_corr_sig.DT, file=file_name, sheetName="pheno_binned_corrs", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(corr_impact_ratios_summary, file=file_name, sheetName="corr_impact_ratios_summary", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(omega_regression_estimates, file=file_name, sheetName="omega_regression_estimates", append=TRUE, row.names=FALSE)
write.xlsx_wrapper(paths_summary, file=file_name, sheetName="paths_summary", append=TRUE, row.names=FALSE)
