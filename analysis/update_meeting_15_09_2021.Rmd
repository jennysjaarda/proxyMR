...
- See [phenotype selection](phenotpye_selection.html) document for complete summary of selection and remaining phenotypes. 
...
```{r corr_traits_summary, include = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE)}
...
```{r PC_corr_couples, include = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE)}
...
```{r PC_traits_corr, include = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE)}

## merge the list of dataframes 
PCs_trait_corr.DT <- PC_trait_corr %>% mutate(r = sqrt(as.numeric(r2))) 
DT::datatable(PCs_corr.DT, width = "100%")  %>% formatSignif(columns=c('r2', 'r'), digits=3)
...
```{r PC_trait_plot, include = FALSE, eval = ifelse(Sys.info()["sysname"]=="Linux", TRUE, FALSE)}

plot <- ggplot2::ggplot(data = corr_impact_by_PCs, ggplot2::aes(x = trait_couple_corr,
                                                                y = corr_due_to_confounding, color = factor(overlap_above_50))) +
