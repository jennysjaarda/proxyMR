...
Each phenotype in the SGG database was filtered for the following criteria based on the genetic qc file (found here `r files_ref %>% filter(custom_names == "sqc") %>% pull(files)`: 
...
Household couples defined above were filtered based on kinship (estimate of the kinship coefficient for pair based on the set of markers used in the kinship inference) from the following file: `r tar_read(path_relatives)`. Couples were excluded if kinship was > `0.05`. 
...
Finally, using the data at data-field `6141`, ["How are people in household related to participant"](https://biobank.ctsu.ox.ac.uk/showcase/field.cgi?id=6141), heterosexual, unrelated, household pairs were filtered to only include couples who both said: `Husband, wife, or partner`. 
