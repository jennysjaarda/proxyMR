...
- For certain analyses it is worth only looking at MR results between couples where the exposure trait is the same as the outcome trait. 
- These results we have termed, the assortative mating MR results. 
...
- `IVW_meta_beta` and `IVW_meta_pval` corresponds to the beta and p-value of the meta-analyzed MR across sexes, respectively.  
- After adjusting for multiple tests (p < `0.05 / `r num_tests_by_PCs``), the number of significant AM MRs was `r length(AM_sig_traits)` (see below on details for how number of effective tests was obtained).
...
- A similar adjustment for multiple hypothesis testing was applied as above, resulting in the number of effective tests as `r num_tests_by_PCs_AM_sig`. 
...
- Only `r dim(AM_result_sig.DT)[1]/2` was significant after adjustment. 
- The table below shows the results significant at `p < 0.05`.
...
- For each AM MR ($X_{p} \sim X_i$), MR analyses were run in the full sample as well as in 5 roughly equal sized bins according to time couple had been together (`time_together`) estimated using [time at household variable](https://biobank.ctsu.ox.ac.uk/showcase/field.cgi?id=699), and median age (`age`). 
...
- Specifically, the outcome data set was split into 5 bins, and the SNP-outcome effect was estimated in each bin separately. 
...
- The two results are shown below. Note that each statistic was computed in each sex separately and meta-analyzed. 
...
The number of significant q-statistics significant after multiple hypothesis testing (p < `0.05/`r num_tests_by_PCs_AM_sig` = `r 0.05/num_tests_by_PCs_AM_sig```) in each group was:
  - Binned by median *age* of couples, number of significant results within meta = `r length(which(q_het_result_age$Q_pval_meta < 0.05/num_tests_by_PCs_AM_sig))/2` and sex-specific = `r length(which(q_het_result_age$Q_pval < 0.05/num_tests_by_PCs_AM_sig))/2`.
  - Binned by *time* couple had been together, number of significant results within meta = `r length(which(q_het_result_tt$Q_pval_meta < 0.05/num_tests_by_PCs_AM_sig))/2` and sex-specific = `r length(which(q_het_result_tt$Q_pval < 0.05/num_tests_by_PCs_AM_sig))/2`.
...
Slopes were calculated by estimating the beta-coefficient of a linear regression between MR estimate within each bin (outcome) and median bin (either age or time at same household). Linear models were run both *unweighted* and *weighted* for the by the inverse of the SE of the MR estimate. 

The number of significant slopes ($MR_{bin} \sim median_{bin}$) significant after multiple hypothesis testing (p < `0.05/`r num_tests_by_PCs_AM_sig` = `r 0.05/num_tests_by_PCs_AM_sig```) in each group was:
  - Binned by median *age* of couples, number of significant results within meta = `r length(which(slope_result_age$bin_slope_meta_pval < 0.05/num_tests_by_PCs_AM_sig))/2`, weighted by inverse of the SE = `r length(which(slope_result_age$bin_slope_meta_pval_wt < 0.05/num_tests_by_PCs_AM_sig))/2`, different between sexes = `r length(which(slope_result_age$bin_slope_sex_het_pval < 0.05/num_tests_by_PCs_AM_sig))/2`
  - Binned by *time* couple had been together, number of significant results within meta = `r length(which(slope_result_tt$bin_slope_meta_pval < 0.05/num_tests_by_PCs_AM_sig))/2` and weighted by inverse of the SE = `r length(which(slope_result_tt$bin_slope_meta_pval_wt < 0.05/num_tests_by_PCs_AM_sig))/2`, different between sexes = `r length(which(slope_result_tt$bin_slope_sex_het_pval < 0.05/num_tests_by_PCs_AM_sig))/2`
All results are displayed below (sort by Q_pval to view significant results):
